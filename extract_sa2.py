...
out_path = "D:/Hackathon_work/sanantonio_sampled.geojson"
TARGET = "sanantonio"
SAMPLE_RATE = 0.08   # keep ~8% of small buildings -> file stays small
    ...
    with open(in_path, "r", encoding="utf-8") as f:
        for feat in ijson.items(f, "features.item"):
            ...
            if not (-107 < lng < -93 and 25 < lat < 37) or carbon <= 0:
                broken += 1; continue
            # keep all big buildings, sample the small ones
            if area < 2000 and random.random() > SAMPLE_RATE:
                continue
            key = (round(lng, 5), round(lat, 5), round(area, 1))
            if key in keys:
                dupes += 1; continue
            keys.add(key)
            if kept > 0:
